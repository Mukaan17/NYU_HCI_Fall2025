...
actor APIService {
    ...
    struct OAuthAuthorizationResponse: Codable, Sendable {
    ...
    nonisolated func getGoogleCalendarAuthURL(jwt: String) async throws -> OAuthAuthorizationResponse {
        guard let url = URL(string: "\(baseURL)/api/calendar/oauth/authorize") else {
            throw APIError.invalidURL
        }
        ...
        let decoder = JSONDecoder()
        return try decoder.decode(OAuthAuthorizationResponse.self, from: data)
    }
    
    nonisolated func unlinkGoogleCalendar(jwt: String) async throws {
        guard let url = URL(string: "\(baseURL)/api/calendar/oauth/unlink") else {
            throw APIError.invalidURL
        }
